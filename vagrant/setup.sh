...
## Link Codebase
if [[ ! -d ~/phantom-deployer/ ]]; then
    ln -s /vagrant ~/phantom-deployer
fi
...
~/phantom-deployer/bridgechain.sh start-node --name "$CHAIN_NAME" &>> ~/node.log &
~/phantom-deployer/bridgechain.sh start-explorer &>> ~/explorer.log &
EOS
